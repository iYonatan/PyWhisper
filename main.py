...
print "MEMORY has been edited"

disk = disk()
print disk.devices
for d in range(len(disk.devices)):
    add_disk = "SELECT user_id,device,mountpoint FROM disk WHERE (user_id=%s AND device=%s AND mountpoint=%s)"
    data = (user_id,disk.devices[d],disk.devices[d])
